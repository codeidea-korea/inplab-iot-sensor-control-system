...
<html lang="ko">
    <head>
        ...
        <script>
			...
			const actionDelCctv = (dispbd_no) => {
				udtCctv([{dispbd_no: dispbd_no, del_yn: 'Y'}]).then((res) => {
					if (res.count?.pass > 0) {
					...
					alert2('CCTV 정보가 삭제되었습니다.', function () {
						...
						const search_dispbd_nm = $('input[name=search_dispbd_nm]').val();
						offset = 0;
						getCctv({dispbd_nm: search_dispbd_nm, limit: limit, offset: offset}).then((res) => {
            ...
            $(function () {
				...
				$('.searchtBtn').on('click', function() {
					const dispbd_nm = $('input[name=search_dispbd_nm]').val();
					offset = 0;
					getCctv({dispbd_nm : dispbd_nm, limit : limit, offset : offset}).then((res) => {
						const formattedData = actFormattedData(res.rows, 'dispbd_no');
						$("#jqGrid").jqGrid('clearGridData');
						$("#jqGrid").jqGrid('setGridParam', {data: formattedData}).trigger('reloadGrid');
					}).catch((fail) => {
                ...
                $('.modifyBtn').on('click', function() {
					...
					Promise.all([getCctv({dispbd_no : dispbd_no}), getDistrictInfo(), getMaintCompInfo({partner_type_flag: '1'})]).then(([res1, res2, res3]) => {
				...
				$('input[name=search_dispbd_nm]').on('keypress', function(e) {
					if (e.key === 'Enter') {
