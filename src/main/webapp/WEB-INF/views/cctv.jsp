...
<head>
    ...
    <style>
        ...
        .cctvzoom .cctvcontrol button:active {
            background-color: #00000090; /* 눌렸을 때 배경 색 */
            transform: scale(0.9); /* 살짝 작아지는 효과 */
        }
    ...
    <script>
        ...
        const setCctvVideoList = (data) => {
            const html =
                ...
                '        <button data-operator="down" data-cctv-no=' + data.cctv_no + ' type="button" class="operator-btn">▼</button> ' +
                '        <button data-operator="right" data-cctv-no=' + data.cctv_no + ' type="button" class="operator-btn">▶</button> ' +
                '        <button data-operator="plus" data-cctv-no=' + data.cctv_no + ' type="button" class="operator-btn">+</button> ' +
                '        <button data-operator="minus" data-cctv-no=' + data.cctv_no + ' type="button" class="operator-btn">-</button> ' +
                '    </div>' +
                '</div>'
            '</div>' +
            '</li>';
            ...
            $(".operator-btn").click(function () {
                ...
                $.ajax("/modify/cctv/operation?operator=" + operator + "&cctv_no=" + cctvNo, (res) => {
                    console.log(res);
                });
            });
...
<section id="wrap">
    ...
    <div id="container">
        ...
        <div id="contents" class="cctvList">
            ...
            <div class="contents-re">
                <h3 class="txt">현장 CCTV
                    <div class="visionBtn"><i class="fa-solid fa-tv"></i> &nbsp; 전체화면</div>
                </h3>
                <div class="contents-in">
                    <div class="scrollY">
                        <ul class="cctv-list">
