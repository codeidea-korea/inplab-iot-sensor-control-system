...
<%--                <li fc_step5 asset_id=""><span>기타</span> <strong>1개</strong></li>--%>
    ...
    <div class="site-status-details_re" two="">
        ...
        <div class="conts">
            <div class="noTable maintenance-list">
                <table>
<%--                    <thead>--%>
<%--                        <tr>--%>
<%--                            <th>종류</th>--%>
...
<script>
    ...
    $(function() {
        ...
        // 장비 현황 클릭시
        $('.site-status-details_re .equipment-list li').off().on('click', function () {
            let assetKindId = $(this).attr('kind');
        ...
        function loadDeviceCount() {
            $.get('/deviceCount', param, function (res) {
                ...
                for (let i = 0; i < values.length; i++) {
                    if (values[i] > 0) {
                        $('.site-status-details_re .equipment-list li:eq(' + i + ')').css("cursor", "pointer");
                    } else {
                        $('.site-status-details_re .equipment-list li:eq(' + i + ')').off('click');
                    }
                    $('.site-status-details_re .equipment-list li:eq(' + i + ') strong').html(values[i] + "개");
                }
            });
        }
        ...
        function loadMaintenance() {
            $.get('/maintenanceInfo', param, function (res) {
                // console.log(res);
                if (typeof res != 'undefined') $('.site-status-details_re .maintenance-list tbody').empty();
                if (res.length > 0) $('.site-status-details_re .maintenance-list').css("cursor", "pointer");
                $.each(res, function (idx) {
                    let contents = '<tr>';
                    contents += '<td><div class="stats">' + res[idx].type_name+ '</div></td>';
                    contents += '<td>' + res[idx].asset_name+ '</td>';
                    contents += '<td>' + res[idx].manager_name+ '</td>';
                    contents += '</tr>';

                    $('.site-status-details_re .maintenance-list tbody').append(contents);
