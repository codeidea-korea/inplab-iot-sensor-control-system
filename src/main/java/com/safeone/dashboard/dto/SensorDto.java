...
public class SensorDto implements Serializable {
    ...
    @FieldLabel(title="센서 종류", width=200)
    String asset_kind_id;
    
    @FieldLabel(title="센서 종류", type="hidden")
    String asset_kind_id_hid;

    @FieldLabel(title="센서명", width=200)
    String name;

    @FieldLabel(title="설치 일자", width=200, type = "range")
    String install_date;
    
    @FieldLabel(title="최종 측정 일시", width=350, type = "timestamp_range")
    String collect_date;
    
    @FieldLabel(title="계측기 최종 측정 일시", width=350, type = "timestamp_range")
    String ch_collect_date;
    
    @FieldLabel(title="상태")
    String status;

    @FieldLabel(title="센서값", width=200)
    String real_value;
